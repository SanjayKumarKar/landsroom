...
//Set Common things for this controller*************************************************************
	...
	function  __construct() {
        parent::__construct();
        $this->COMMON['DB']='tb_redirection';
        $this->COMMON['ID_FIELD']='redirection_id';
        $this->COMMON['VIEW_CONTROLLER']='redirection';
...
//Initialize Common Details
	private function initData()
	{
		...
		$this->common_data['title'] 			= 	'Redirection';
...
//Add Form Submission	
	public function addSubmitted($CALL_TYPE='NORMAL')
	{
		...
		$this->form_validation->set_rules('redirection_old_url','Old URL','required');
		$this->form_validation->set_rules('redirection_new_url','New URL','required');
		//Run Validation to Check form validation
		if($this->form_validation->run()) 
		{
			//Set Data into array
			$insertData = array(
					'redirection_old_url' => urlencode($this->input->post('redirection_old_url')),
					'redirection_new_url' => urlencode($this->input->post('redirection_new_url'))
			);
...
//Data Fetch**************************************************************************	
	 ...
	 public function getData()
	 {
			...
			$searColumns = array('redirection_old_url','redirection_new_url');
			...
			if(count($results['DATA'])>0){
				foreach ($results['DATA'] as $DATA) {
					...
					$nestedData[] = '<a href="'.front_base_url($DATA['redirection_old_url']).'" target="_blank">'.front_base_url($DATA['redirection_old_url']).'</a>';
					$nestedData[] = '<a href="'.front_base_url($DATA['redirection_new_url']).'" target="_blank">'.front_base_url($DATA['redirection_new_url']).'</a>';
