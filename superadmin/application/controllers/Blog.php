...
//Set Common things for this controller*************************************************************
	...
	function  __construct() {
        parent::__construct();
        $this->COMMON['DB']='tb_blog';
        $this->COMMON['ID_FIELD']='blog_id';
        $this->COMMON['VIEW_CONTROLLER']='blog';
...
//Initialize Common Details
	private function initData()
	{
		...
		$FIELD='*';
		$CON='IsDel="0"';
		$ORDER=array('field'=>'blog_category_title','direction'=>'asc');
		$this->common_data['BLOG_CATEGORY']=$this->FetchModel->SelectDB($FIELD,'tb_blog_category',$CON,$ORDER);
		
		
		// //Run SelectDB Function which select data from database
		// $FIELD='*';
		// $CON='IsDel="0"';
		// $ORDER=array('field'=>'author_name','direction'=>'asc');
		// $this->common_data['BLOG_AUTHOR']=$this->FetchModel->SelectDB($FIELD,'tb_author',$CON,$ORDER);
		
		// //Run SelectDB Function which select data from database
		// $FIELD='*';
		// $CON='IsDel="0" and doctor_is_author="1"';
		// $ORDER=array('field'=>'doctor_name','direction'=>'asc');
		// $this->common_data['BLOG_DOCTOR_AUTHOR']=$this->FetchModel->SelectDB($FIELD,'tb_doctor',$CON,$ORDER);
		
		// //Run SelectDB Function which select data from database
		// $FIELD='*';
		// $CON='IsDel="0" and doctor_is_reviewer="1"';
		// $ORDER=array('field'=>'doctor_name','direction'=>'asc');
		// $this->common_data['BLOG_DOCTOR_REVIEWER']=$this->FetchModel->SelectDB($FIELD,'tb_doctor',$CON,$ORDER);
...
//Add Form Submission	
	public function addSubmitted()
	{
		...
		$this->form_validation->set_rules('blog_slug','Slug','required');
		//Run Validation to Check form validation
		if($this->form_validation->run()) 
		{
			//Set Data into array
			$insertData = array(
					'blog_blog_category_id' => implode(",",$this->input->post('blog_blog_category_id')),
					'blog_speciality' => empty($this->input->post('blog_speciality'))?"0":implode(",",$this->input->post('blog_speciality')),
					'blog_procedure' => empty($this->input->post('blog_procedure'))?"0":implode(",",$this->input->post('blog_procedure')),
					'blog_disease' => empty($this->input->post('blog_disease'))?"0":implode(",",$this->input->post('blog_disease')),
					'blog_country' => empty($this->input->post('blog_country'))?"0":implode(",",$this->input->post('blog_country')),
					'blog_state' => empty($this->input->post('blog_state'))?"0":implode(",",$this->input->post('blog_state')),
					'blog_city' => empty($this->input->post('blog_city'))?"0":implode(",",$this->input->post('blog_city')),
					'blog_doctor' => $this->input->post('blog_doctor'),
					'blog_hospital' => $this->input->post('blog_hospital'),
					'blog_title' => $this->input->post('blog_title'),
					'blog_slug' => $this->input->post('blog_slug'),
					'blog_date' => $this->input->post('blog_date'),
					'blog_short_details' => $this->input->post('blog_short_details'),
					'blog_details' => $this->input->post('blog_details',false),
					'blog_image' => $this->input->post('blog_image'),
					'blog_author' => $this->input->post('blog_author'),
					'blog_author_doctor' => $this->input->post('blog_author_doctor'),
					'blog_reviewer_doctor' => $this->input->post('blog_reviewer_doctor'),
					...
					'blog_og_tag' => $this->input->post('blog_og_tag',false),
					'blog_schema' => $this->input->post('blog_schema',false)
			);
...
//Data Fetch**************************************************************************	
	 ...
	 public function getData()
	 {
			...
			$searColumns = array('blog_date','blog_title');
			...
			if(count($results['DATA'])>0){
				foreach ($results['DATA'] as $DATA) {
					...
					$nestedData[] = date("d-m-Y",strtotime($DATA['blog_date']));
					$nestedData[] = $DATA['blog_title'];
...
//Extra Code**************************************************************************
	//Check Blog Slug Existance and Generate Alternative
	public function checkBlogSlug()
	{
		$slug=$this->input->post('blog_slug');
		$slug_prev=$this->input->post('blog_slug_prev');
		if($slug!=$slug_prev)
		{
			...
			if(count($RET_VALUE)==0)
				$VALUE=array('blog_slug'=>$slug);
			else
			{
				//Run SelectDBCount Function which get number of row from database
				$FIELD='blog_id';
				$CON='blog_slug like "'.$slug.'%"';
				$RET_VALUE=$this->FetchModel->SelectDB($FIELD,$this->COMMON['DB'],$CON);
