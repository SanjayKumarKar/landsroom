...
//Set Common things for this controller*************************************************************
	...
	function  __construct() {
        parent::__construct();
        $this->COMMON['DB']='tb_city';
        $this->COMMON['ID_FIELD']='city_id';
        $this->COMMON['VIEW_CONTROLLER']='city';
...
//Add Form Submission	
	public function addSubmitted()
	{
		...
		$this->form_validation->set_rules('city_name','Title','required');
		$this->form_validation->set_rules('city_slug','Slug','required');
		...
		if($this->form_validation->run()) 
		{
			...
			$insertData = array(
					...
					'city_slug' => $this->input->post('city_slug'),
					...
					'city_show_in_featured' => $this->input->post('city_show_in_featured'),
					'city_order_featured' => $this->input->post('city_order_featured'),
					'city_order_master' => $this->input->post('city_order_master')
			);
...
//Edit View**********************************************************************************	
	public function edit($id=0)
	{
		...
		//Run SelectDB Function which select data from database
		$FIELD='*';
		$CON='IsDel="0" and state_country_id='.$this->common_data['list'][0]['city_country_id'];
		$ORDER=array('field'=>'state_name','direction'=>'asc');
		$this->common_data['STATE']=$this->FetchModel->SelectDB($FIELD,'tb_state',$CON,$ORDER);
...
//Data Fetch**************************************************************************	
	 ...
	 public function getData()
	 {
			...
			$searColumns = array('city_id','city_name','country_name','state_name');
			if( !empty($params['search']['value']) ) {
		 	...
		 	//Join Tables(Change)
			$JOIN=array(
				array('table'=>'tb_country', 'join_key'=>'tb_country.country_id = tb_city.city_country_id'),
				array('table'=>'tb_state', 'join_key'=>'tb_state.state_id = tb_city.city_state_id')
			);
			...
			if(count($results['DATA'])>0){
				foreach ($results['DATA'] as $DATA) {
					...
					$nestedData[] = $DATA['city_id'];
					$nestedData[] = $DATA['city_name'];
					$nestedData[] = $DATA['country_name'];
					$nestedData[] = $DATA['state_name'];
					$nestedData[] = $DATA['city_status']==0?'<span class="badge badge-danger">Inactive</span>':'<span class="badge badge-success">Active</span>';
					$nestedData[] = $DATA['city_show_in_featured']==0?'<span class="badge badge-danger">No</span>':'<span class="badge badge-success">Yes</span>';
...
//Extra Code**************************************************************************
	//Check City Slug Existance and Generate Alternative
	public function checkCitySlug()
	{
		$slug=$this->input->post('city_slug');
		$slug_prev=$this->input->post('city_slug_prev');
		if($slug!=$slug_prev)
		{
			...
			if(count($RET_VALUE)==0)
				$VALUE=array('city_slug'=>$slug);
			else
			{
				//Run SelectDBCount Function which get number of row from database
				$FIELD='city_id';
				$CON='city_slug like "'.$slug.'%"';
				$RET_VALUE=$this->FetchModel->SelectDB($FIELD,$this->COMMON['DB'],$CON);
...
//Import Form Submission	
			...
			for($row=2; $row<=$highestRow; $row++)
			{
				if(!empty($worksheet->getCellByColumnAndRow(0, $row)->getValue()))
				{
					...
					$insertData = array(
							...
							'city_slug' => $this->getSlug($worksheet->getCellByColumnAndRow(0, $row)->getValue()),
							'city_country_id' => $worksheet->getCellByColumnAndRow(1, $row)->getValue(),
							'city_state_id' => $worksheet->getCellByColumnAndRow(2, $row)->getValue(),
							'city_show_in_featured' => $worksheet->getCellByColumnAndRow(3, $row)->getValue(),
							'city_order_featured' => $worksheet->getCellByColumnAndRow(4, $row)->getValue(),
							'city_order_master' => $worksheet->getCellByColumnAndRow(5, $row)->getValue(),
							'city_status' => $worksheet->getCellByColumnAndRow(6, $row)->getValue()
					);
					//Run InsertDB Function which insert data into database
					$RET_VALUE=$this->AddModel->InsetDB($this->COMMON['DB'], $insertData);
					$COUNT++;
	...
	public function getCity()
	{
		...
		//Run SelectDB Function which select data from database
		$FIELD='*';
		$CON='IsDel="0" and city_state_id IN ('.$check_data.')';
		$ORDER=array('field'=>'city_name','direction'=>'asc');
		$this->common_data['DATA']=$this->FetchModel->SelectDB($FIELD,'tb_city',$CON,$ORDER);
