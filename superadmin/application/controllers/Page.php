...
//Set Common things for this controller*************************************************************
	...
	function  __construct() {
        parent::__construct();
        $this->COMMON['DB']='tb_page';
        $this->COMMON['ID_FIELD']='page_id';
	...
	public function addSubmitted()
	{
		...
		if($this->form_validation->run()) 
		{
			...
			$insertData = array(
					...
					'page_parameter' => $this->input->post('page_parameter'),
					'page_slug' => $this->input->post('page_slug'),
					'page_details' => $this->input->post('page_details',false),
					...
					'page_data' => json_encode($this->input->post('page_data',false)),
					'page_og_tag' => $this->input->post('page_og_tag',false),
					'page_schema' => $this->input->post('page_schema',false)
			);
	 ...
	 public function getData()
	 {
			...
			if(count($results['DATA'])>0){
				foreach ($results['DATA'] as $DATA) {
					...
					$nestedData[] = $DATA['page_title'];
					$nestedData[] = '<a href="'.front_base_url($DATA['page_slug']).'" target="_blank">'.front_base_url($DATA['page_slug']).'</a>';
...
//Extra Code**************************************************************************
	//Check Page Slug Existance and Generate Alternative
	public function checkPageSlug()
	{
		$slug=$this->input->post('page_slug');
		$slug_prev=$this->input->post('page_slug_prev');
		if($slug!=$slug_prev)
		{
			...
			if(count($RET_VALUE)==0)
				$VALUE=array('page_slug'=>$slug);
			else
			{
				//Run SelectDBCount Function which get number of row from database
				$FIELD='page_id';
				$CON='page_slug like "'.$slug.'%"';
				$RET_VALUE=$this->FetchModel->SelectDB($FIELD,$this->COMMON['DB'],$CON);
	...
	public function getTemplate()
	{
		...
		//$template='Vision_and_mision_template';
		
		//Run SelectDB Function which select data from database
		$FIELD='page_data';
		$CON='page_template="'.$template.'"';
		$ORDER=array('field'=>$this->COMMON['ID_FIELD'],'direction'=>'desc');
		$this->common_data['list']=$this->FetchModel->SelectDB($FIELD,$this->COMMON['DB'],$CON,$ORDER);
		
		$VALUE=array(
		"template_view"=>$this->load->view('modules/'.$this->COMMON['VIEW_CONTROLLER'].'/template/'.$template, $this->common_data, true)
		);
		echo json_encode($VALUE);
	}
