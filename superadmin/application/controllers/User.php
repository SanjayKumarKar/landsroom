...
//Set Common things for this controller*************************************************************
	...
	function  __construct() {
        parent::__construct();
        $this->COMMON['DB']='tb_user';
        $this->COMMON['ID_FIELD']='user_id';
        $this->COMMON['VIEW_CONTROLLER']='user';
...
//Add Form Submission	
	public function addSubmitted()
	{
        ...
        $this->form_validation->set_rules('user_contact_number', 'Contact Number', 'required');
...
//Edit Form Submission	
	public function editSubmitted()
	{
		...
		if($this->form_validation->run()) 
		{
			...
			$CON=$this->COMMON['ID_FIELD'].'='.$this->input->post('user_id');
			...
			else
			{
				...
				return redirect(base_url($this->COMMON['VIEW_CONTROLLER'].'/edit/'.$this->input->post('user_id')));
			}
		}
		else
		{
...
//Data Fetch**************************************************************************	
	 ...
	 public function getData()
	 {
			...
			$searColumns = array('user_type','user_name');
			...
			if(count($results['DATA'])>0){
				foreach ($results['DATA'] as $DATA) {
					...
					$nestedData[] = $DATA['user_name'];
					$nestedData[] = $DATA['user_contact_number'];
                    $userTypes = [
                        0 => "Buyer",
                        1 => "Owner",
                        2 => "Tenant",
                    ...
                    $nestedData[] = isset($userTypes[$DATA['user_type']]) ? $userTypes[$DATA['user_type']] : "Unknown";
                    
                    $nestedData[] = $DATA['user_email'];
                    $nestedData[] = $DATA['user_dob'];
                    $nestedData[] = $DATA['user_state'];
                    $nestedData[] = $DATA['user_country'];
                    $nestedData[] = $DATA['user_city'];
                    $nestedData[] = $DATA['user_address'];
                    $nestedData[] = $DATA['user_pin'];
                    $userGender = [
                        0 => "Female",
                        1 => "Male",
                        2 => "Others",
                    ];
                    $nestedData[] = isset($userGender[$DATA['user_gender']]) ? $userGender[$DATA['user_gender']] : "Unknown";
                    $nestedData[] = '<img src="'. file_upload_base_url($list[0] . ($DATA['user_profile_picture'])) . '" width="30%">';
                    // $nestedData[] = $DATA[''];
