...
//Set Common things for this controller*************************************************************
	...
	function  __construct() {
        parent::__construct();
        $this->COMMON['DB']='tb_support';
        $this->COMMON['ID_FIELD']='support_id';
        $this->COMMON['VIEW_CONTROLLER']='support';
...
//Initialize Common Details
	private function initData()
	{
		...
		$this->common_data['title'] 			= 	'Support';
...
//Add Form Submission	
	public function addSubmitted()
	{
		...
		$this->form_validation->set_rules('support_contact','Contact Number','required');
...
//Data Fetch**************************************************************************	
	 ...
	 public function getData()
	 {
			...
			$searColumns = array('support_id','support_name','support_contact','support_email','support_subject','support_message','support_source','createdOn');
			...
			if(count($results['DATA'])>0){
				foreach ($results['DATA'] as $DATA) {
					...
					$nestedData[] = $DATA['createdOn'];
					$nestedData[] = $DATA['support_name'];
					$nestedData[] = $DATA['support_contact'];
					$nestedData[] = $DATA['support_email'];
					$nestedData[] = $DATA['support_subject'];
					$nestedData[] = '<a href="'.$DATA['support_source'].'" target="_blank">Click</a>';
		...
		if (in_array($fileMimeType, $allowedMimeTypes) && in_array($fileExtension, $allowedExtensions) && isset($file["name"])) 
		{
			...
			for($row=2; $row<=$highestRow; $row++)
			{
				if(!empty($worksheet->getCellByColumnAndRow(0, $row)->getValue()))
				{
					//Set Data into array
					$insertData = array(
							'support_name' => $worksheet->getCellByColumnAndRow(0, $row)->getValue(),
							'support_contact' => $worksheet->getCellByColumnAndRow(1, $row)->getValue(),
							'support_email' => $worksheet->getCellByColumnAndRow(2, $row)->getValue(),
							'support_subject' => $worksheet->getCellByColumnAndRow(3, $row)->getValue(),
							'support_message' => $worksheet->getCellByColumnAndRow(4, $row)->getValue(),
							'support_source' => $worksheet->getCellByColumnAndRow(5, $row)->getValue()
					);
					//Run InsertDB Function which insert data into database
					$RET_VALUE=$this->AddModel->InsetDB($this->COMMON['DB'], $insertData);
					$COUNT++;
