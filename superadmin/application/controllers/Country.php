...
//Set Common things for this controller*************************************************************
	...
	function  __construct() {
        parent::__construct();
        $this->COMMON['DB']='tb_country';
        $this->COMMON['ID_FIELD']='country_id';
        $this->COMMON['VIEW_CONTROLLER']='country';
...
//Initialize Common Details
	private function initData()
	{
		...
		$this->common_data['title'] 			= 	'Country';
...
//Add Form Submission	
	public function addSubmitted()
	{
		...
		$this->form_validation->set_rules('country_name','Title','required');
		$this->form_validation->set_rules('country_slug','Slug','required');
		...
		if($this->form_validation->run()) 
		{
			...
			$insertData = array(
					...
					'country_show_in_featured' => $this->input->post('country_show_in_featured'),
					'country_order_featured' => $this->input->post('country_order_featured'),
					'country_order_master' => $this->input->post('country_order_master')
			);
...
//Data Fetch**************************************************************************	
	 ...
	 public function getData()
	 {
			...
			$searColumns = array('country_id','country_name','country_short_name','country_phone_code');
			...
			if(count($results['DATA'])>0){
				foreach ($results['DATA'] as $DATA) {
					...
					$nestedData[] = $DATA['country_id'];
					$nestedData[] = $DATA['country_name'];
					$nestedData[] = $DATA['country_short_name'];
					$nestedData[] = $DATA['country_phone_code'];
					$nestedData[] = $DATA['country_status']==0?'<span class="badge badge-danger">Inactive</span>':'<span class="badge badge-success">Active</span>';
					$nestedData[] = $DATA['country_show_in_featured']==0?'<span class="badge badge-danger">No</span>':'<span class="badge badge-success">Yes</span>';
...
//Extra Code**************************************************************************
	//Check Country Slug Existance and Generate Alternative
	public function checkCountrySlug()
	{
		$slug=$this->input->post('country_slug');
		$slug_prev=$this->input->post('country_slug_prev');
		if($slug!=$slug_prev)
		{
			//Run SelectDBCount Function which get number of row from database
			$FIELD='country_id';
			$CON='country_slug="'.$slug.'"';
			$RET_VALUE=$this->FetchModel->SelectDB($FIELD,$this->COMMON['DB'],$CON);
			if(count($RET_VALUE)==0)
				$VALUE=array('country_slug'=>$slug);
			else
			{
				//Run SelectDBCount Function which get number of row from database
				$FIELD='country_id';
				$CON='country_slug like "'.$slug.'%"';
				$RET_VALUE=$this->FetchModel->SelectDB($FIELD,$this->COMMON['DB'],$CON);
			...
			for($row=2; $row<=$highestRow; $row++)
			{
				if(!empty($worksheet->getCellByColumnAndRow(0, $row)->getValue()))
				{
					...
					$insertData = array(
							...
							'country_slug' => $this->getSlug($worksheet->getCellByColumnAndRow(0, $row)->getValue()),
							'country_short_name' => $worksheet->getCellByColumnAndRow(1, $row)->getValue(),
							'country_phone_code' => $worksheet->getCellByColumnAndRow(2, $row)->getValue(),
							'country_status' => $worksheet->getCellByColumnAndRow(6, $row)->getValue(),
							'country_show_in_featured' => $worksheet->getCellByColumnAndRow(3, $row)->getValue(),
							'country_order_featured' => $worksheet->getCellByColumnAndRow(4, $row)->getValue(),
							'country_order_master' => $worksheet->getCellByColumnAndRow(5, $row)->getValue()
					);
					//Run InsertDB Function which insert data into database
					$RET_VALUE=$this->AddModel->InsetDB($this->COMMON['DB'], $insertData);
					$COUNT++;
