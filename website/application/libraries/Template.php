...
class Template
{
    ...
    function set_template($group)
    {
        if (isset($this->config[$group])) {
            ...
            show_error(
                'The "' .
                    $group .
                    '" template group does not exist. Provide a valid group name or add the group first.'
            );
        }
    ...
    {
        if (
            file_exists(APPPATH . "views/" . $filename) or
            file_exists(APPPATH . "views/" . $filename . EXT)
        ) {
            $this->master = $filename;
        } else {
            show_error(
                "The filename provided does not exist in <strong>" .
                    APPPATH .
                    'views</strong>. Remember to include the extension if other than ".php"'
            );
        }
    }
    ...
    function add_template($group, $template, $activate = false)
    {
        if (!isset($this->config[$group])) {
            ...
            show_error(
                'The "' .
                    $group .
                    '" template group already exists. Use a different group name.'
            );
        }
    }
    ...
    function initialize($props)
    {
        ...
        if (
            ...
            (file_exists(APPPATH . "views/" . $props["template"]) or
                file_exists(APPPATH . "views/" . $props["template"] . EXT))
        ) {
            ...
            show_error(
                "Either you have not provided a master template or the one provided does not exist in <strong>" .
                    APPPATH .
                    'views</strong>. Remember to include the extension if other than ".php"'
            );
        }
    ...
    function add_region($name, $props = [])
    {
        if (!is_array($props)) {
            $props = [];
        }
    ...
    function write($region, $content, $overwrite = false)
    {
        if (isset($this->regions[$region])) {
            if ($overwrite === true) {
                // Should we append the content or overwrite it
        ...
        // Regions MUST be defined
            ...
            show_error(
                "Cannot write to the '{$region}' region. The region is undefined."
            );
        }
    }
    ...
    function write_view($region, $view, $data = null, $overwrite = false)
    {
        ...
        if (count($args) > 1) {
            foreach ($args as $suggestion) {
                if (
                    file_exists(APPPATH . "views/" . $suggestion . EXT) or
                    file_exists(APPPATH . "views/" . $suggestion)
                ) {
		...
		// $content = $this->modifyImageTags($content);
    ...
    function parse_view($region, $view, $data = null, $overwrite = false)
    {
        ...
        $content = $this->CI->{$this->parser}->{$this->parser_method}(
            $view,
            $data,
            true
        );
    ...
    function render($region = null, $buffer = false, $parse = false)
    {
        ...
        if ($region) {
            // Display a specific regions contents

            if (isset($this->regions[$region])) {
                $output = $this->_build_content($this->regions[$region]);
            } else {
                show_error(
                    "Cannot render the '{$region}' region. The region is undefined."
                );
            }
        }
        ...
        // Build the output array
            ...
            if ($this->parse_template === true or $parse === true) {
                ...
                $output = $this->CI->{$this->parser}->{$this->parser_method}(
                    $this->master,
                    $this->output,
                    true
                );
    ...
    function _build_content($region, $wrapper = null, $attributes = null)
    {
        ...
        if (isset($region["wrapper"])) {
            ...
            $output .= substr(
                $region["wrapper"],
                0,
                strlen($region["wrapper"]) - 1
            );

            // Add HTML attributes

            if (
                isset($region["attributes"]) &&
